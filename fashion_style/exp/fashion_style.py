...
with tf.Session(graph=graph, config=config) as session:
    ...
    train_features = tf.compat.v1.constant(train_features, dtype=tf.float32, shape=train_features.shape)
    test_features = tf.compat.v1.constant(test_features, dtype=tf.float32, shape=test_features.shape)
    ...
    train_features_w1 = tf.sigmoid(tf.matmul(train_features, encoder_w1) + encoder_b1).eval()
    train_features_new = tf.sigmoid(tf.matmul(train_features_w1, encoder_w2) + encoder_b2).eval()

    test_features_w1 = tf.sigmoid(tf.matmul(test_features, encoder_w1) + encoder_b1).eval()
    test_features_new = tf.sigmoid(tf.matmul(test_features_w1, encoder_w2) + encoder_b2).eval()
    ...
    for index in [0, 1]:
        ...
        regressor = tf.contrib.learn.DNNRegressor(feature_columns=feature_cols[:, index], hidden_units=n_hidden_2)

        # TODO : 9. Train the regressor. (5%)
        # Hint : feed train features and label into input_fn(...)
        regressor.fit(input_fn=lambda: input_fn(train_features_new, train_labels), steps=100)

        # TODO : 10. Evaluate the loss on test set. (5%)
        # Hint : feed test features and label into input_fn(...)
        current_loss = regressor.evaluate(input_fn=lambda: input_fn(test_features_new, test_labels), steps=1)['loss']
